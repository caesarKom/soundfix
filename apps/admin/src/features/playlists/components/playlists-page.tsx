...
export function PlaylistsPage() {
  ...
  // 2. Query for fetching single playlist detailed details (tracks inside)
  ...
  // 3. Mutation for creating a fresh playlist
  const createMutation = useMutation({
    ...
    onError: (error: unknown) => {
      setErrorMessage(error instanceof Error ? error.message : 'Failed to create playlist');
    },
  });

  // 4. Mutation for deleting a playlist entirely
  ...
  const removeTrackMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-playlist-details', selectedPlaylistId] });
    },
  });
  ...
  const handleDeletePlaylist = (id: string, playlistName: string) => {
    if (window.confirm(`Are you absolutely sure you want to delete playlist "${playlistName}"?`)) {
    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">
          Review user-generated collections, deploy featured global playlists, and moderate track compositions.
        </p>
      </div>
        ...
        <section className="p-6 bg-slate-900 border border-slate-800 rounded-xl shadow-xl space-y-5">
          ...
          <form onSubmit={handleCreateSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Playlist Name</label>
              <input
                type="text"
                value={name}
            ...
            <div className="flex items-center gap-3 py-1">
              ...
              />
              <label htmlFor="isPrivate" className="text-sm font-semibold text-slate-300 cursor-pointer select-none">
            ...
            >
              {createMutation.isPending ? 'Deploying...' : 'Publish Global Playlist'}
            </button>
          </form>
        </section>
        ...
        <section className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-xl overflow-hidden">
            <div className="p-5 border-b border-slate-800 bg-slate-900/50">
              <h2 className="text-lg font-bold text-white tracking-tight">System Playlists Registry</h2>
            </div>

            <div className="overflow-x-auto">
              {isLoading ? (
                <p className="p-8 text-center text-sm text-emerald-400 font-medium animate-pulse">Loading playlists base...</p>
              ) : isError ? (
                <p className="p-8 text-center text-sm text-red-400 font-medium">Failed to look up playlists record.</p>
              ) : (
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-800 text-slate-500 text-xs font-bold uppercase tracking-wider bg-slate-950/20">
                      ...
                      <th className="py-3.5 px-5 text-right">Actions Management</th>
                    </tr>
                  </thead>
                      ...
                      playlists.map((playlist) => (
                        <tr 
                          ...
                          className={`hover:bg-slate-800/10 cursor-pointer transition-colors ${
                            selectedPlaylistId === playlist.id ? 'bg-slate-800/40 border-l-2 border-emerald-500' : ''
                          }`}
                          onClick={() => setSelectedPlaylistId(playlist.id)}
                        >
                          <td className="py-3.5 px-5">
                            <div>
                              <p className="font-bold text-white leading-tight">{playlist.name}</p>
                              <p className="text-xs text-slate-400 mt-0.5 truncate max-w-[240px]">
                                {playlist.description || <span className="italic text-slate-600">No description provided</span>}
                              </p>
                            </div>
                          </td>
                          <td className="py-3.5 px-5">
...
>
  ⚙ Manage Playlist Composition
              ...
              <div className="p-4">
                {isLoadingDetails ? (
                  <p className="text-sm text-center text-slate-500 py-6 animate-pulse">Inspecting tracks...</p>
                ) : activePlaylist?.songs && activePlaylist.songs.length > 0 ? (
                  <div className="space-y-2">
                    {activePlaylist.songs.map((song) => (
                      <div key={song.id} className="flex items-center justify-between p-3 bg-slate-950/40 border border-slate-800/60 rounded-lg hover:border-slate-700 transition-colors">
                        <div>
                          <p className="text-sm font-bold text-white">{song.title}</p>
                          <p className="text-xs text-slate-400 mt-0.5">{song.artist}</p>
                        </div>
                        <button
                          onClick={() => handleRemoveTrack(activePlaylist.id, song.id, song.title)}
